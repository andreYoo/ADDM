...
def anomalous_validation_1(shorten=False,DATASET_PATH=None):
    ...
    for i in range(len(ano_dataset)):
        ...
        _grey_img = np.array(img[0, 0].detach().cpu().numpy())
        normalized_image = (_grey_img + 1) * (255/2)  # Scale from -1-1 to 0-255
        normalized_image = normalized_image.astype(np.uint8)  # Convert to uint8
        cv2.imwrite('./ano_image_data/%d.png'%(_cnt_file), normalized_image)
        _cnt_file+=1
...
#                           f'{new["filenames"][0][-9:-4]}/{new["slices"][0].numpy()[slice_number]}/t={timestep}-attemp' \
#                           f't={len(temp) + 1}'
#             #ani.save(output_name + ".gif",writer='imagemagick')
#             dice_data.append(
#                     evaluation.heatmap(img[slice_number, ...].reshape(1, 1, *args["img_size"]), output[-1].to(device),img_mask[slice_number, ...].reshape(1, 1, *args["img_size"]),output_name + ".png")
#                     )
...
#                         unet, img[slice_number, ...].reshape(1, 1, *args["img_size"]),
#                         args, (new["filenames"][0][-9:-4], new["slices"][0].numpy()[slice_number]),
#                         img_mask[slice_number, ...].reshape(1, 1, *args["img_size"]), "gauss",
#                         total_avg=3
#                         )
#                 dice_data.append(dice)
#             elif args["noise_fn"] == "simplex":
#                 dice = diff.detection_B(unet, img[slice_number, ...].reshape(1, 1, *args["img_size"]),args, (new["filenames"][0][-9:-4], new["slices"][0].numpy()[slice_number]),img_mask[slice_number, ...].reshape(1, 1, *args["img_size"]), denoise_fn=args["noise_fn"],total_avg=3)
#                 dice_data.append(dice)
...
def anomalous_metric_calculation(shorten=False,DATASET_PATH=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    """
    Iterates over 4 anomalous slices for each Volume, returning diffused video for it,
    the heatmap of that & detection method (A&B) or C
...
if __name__ == "__main__":
    ...
    DATASET_PATH = './dataset/EdinburghDataset/Anomalous-T1'
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    if str(sys.argv[1]) == "101" or str(sys.argv[1]) == "102" or str(sys.argv[1]) == "103" or str(sys.argv[1]) == \
        ...
        anomalous_validation_1(DATASET_PATH=DATASET_PATH)
        anomalous_metric_calculation(DATASET_PATH=DATASET_PATH)
