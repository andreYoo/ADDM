...
class GaussianDiffusionModel:
    def __init__(
            ...
            noise="simplex",  # gauss / perlin / simplex
            ):
        super().__init__()
        if noise == "gauss":
    ...
    def calc_loss(self, model, x_0, t):
        ...
        x_tmp = self.sample_p(model,x_t,t) 
        
        estimate_noise = model(x_t, t)
        loss = {}
        recon_loss = self.mse_loss(x_0,x_tmp["pred_x_0"])
        if self.loss_type == "l1":
            loss["loss"] = mean_flat((estimate_noise - noise).abs())+recon_loss
        elif self.loss_type == "l2":
            loss["loss"] = mean_flat((estimate_noise - noise).square())+recon_loss
        elif self.loss_type == "hybrid":
            # add vlb term
            loss["vlb"] = self.calc_vlb_xt(model, x_0, x_t, t, estimate_noise)["output"]
            loss["loss"] = loss["vlb"] + mean_flat((estimate_noise - noise).square())+recon_loss
        else:
            loss["loss"] = mean_flat((estimate_noise - noise).square())+recon_loss
