...
def make_pngs_anogan():
    dir = {
        "Train":     "./DATASETS/AnoGAN/Train", "Test": "./DATASETS/AnoGAN/Test",
        "Anomalous": "./dataset/EdinburghDataset/Anomalous-T1"
        }
    slices = {
...
def main(save_videos=True, bias_corrected=False, verbose=0):
    DATASET = "./dataset/EdinburghDataset"
    patients = os.listdir(DATASET)
    for i in [f"{DATASET}/Anomalous-T1/raw_cleaned", f"{DATASET}/Anomalous-T1/mask_cleaned"]:
        try:
            os.makedirs(i)
        except OSError:
            pass
    if save_videos:
        for i in [f"{DATASET}/Anomalous-T1/raw_cleaned/videos", f"{DATASET}/Anomalous-T1/mask_cleaned/videos"]:
            try:
                os.makedirs(i)
            except OSError:
                pass
            ...
            if "COR_3D" in data_folder:
                ...
                for t1 in T1_files:
                    ...
                    if check and t1.endswith(".nii"):
                        ...
                        np.save(
                                f"{DATASET}/Anomalous-T1/raw_cleaned/{patient}.npy", image.astype(
                                        np.float32
                                        )
                                )
                        np.save(
                                f"{DATASET}/Anomalous-T1/mask_cleaned/{patient}.npy", mask.astype(
                                        np.float32
                                        )
                                )
                        if verbose:
                        ...
                        if save_videos:
                            ...
                            ani.save(f"{DATASET}/Anomalous-T1/raw_cleaned/videos/{patient}.mp4")
                            if verbose:
                            ...
                            ani.save(f"{DATASET}/Anomalous-T1/mask_cleaned/videos/{patient}.mp4")
                            if verbose:
                                print(mask.shape)
                                print(f"Saved {DATASET}/Anomalous-T1/raw/videos/{patient}.mp4")
...
def load_datasets_for_test():
    ...
    ano_dataset = AnomalousMRIDataset(
            ROOT_DIR=f'dataset/EdinburghDataset/Anomalous-T1', img_size=args['img_size'],
...
def init_datasets(ROOT_DIR, args):
    training_dataset = MRIDataset(
            ROOT_DIR=f'{ROOT_DIR}dataset/NFBS_Dataset', img_size=args['img_size'], random_slice=args['random_slice']
            )
...
class AnomalousMRIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.resized:
            img_mask = np.load(f"{self.ROOT_DIR}/mask_cleaned/{self.filenames[idx][-9:-4]}-resized.npy")
        else:
            img_mask = np.load(f"{self.ROOT_DIR}/mask_cleaned/{self.filenames[idx][-9:-4]}.npy")
...
if __name__ == "__main__":
    ...
    d_set = AnomalousMRIDataset(
            ROOT_DIR='./dataset/EdinburghDataset/Anomalous-T1', img_size=(256, 256),
            slice_selection="iterateKnown_restricted", resized=False,cleaned=True
            )
    #
    # #d_set = MRIDataset(ROOT_DIR='./dataset/NFBS_Dataset', img_size=(256, 256)            )
    #
