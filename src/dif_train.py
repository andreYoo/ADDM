...
from detection import anomalous_metric_calculation
from detection import anomalous_validation_1, anomalous_metric_calculation
...
def train(training_dataset_loader, testing_dataset_loader, args, resume):
    ...
    for epoch in tqdm_epoch:
        ...
        for i in iters:
            ...
            writer.add_scalar("lr/train",optimiser.param_groups[0]['lr'],step)
            step+=1
            ...
            if step % 200 == 0:
                ...
                training_outputs(
                        diffusion, x, est, noisy, step, row_size, save_imgs=args['save_imgs'],
...
def training_outputs(diffusion, x, est, noisy, epoch, row_size, ema, args, save_imgs=False, save_vids=False):
    ...
    if save_imgs:
        if epoch % 10 == 0:
            ...
            #t = torch.tensor([diffusion.num_timesteps]).to(x.device)
            #pdb.set_trace()
            t = torch.randint(1, diffusion.num_timesteps, (x.shape[0],), device=x.device)
            x_t = diffusion.sample_q(x, t, noise)
            temp = diffusion.sample_p(ema, x_t, t)
            out = torch.cat((x[:row_size, ...].cpu(), temp["sample"][:row_size, ...].cpu(),temp["pred_x_0"][:row_size, ...].cpu()))
            plt.title(f'real,sample,prediction x_0-{epoch}epoch (Sampling rate %d)'%(t))
        else:
        ...
        plt.savefig(f'./diffusion-training-images/ARGS={args["arg_num"]}/STEP={epoch}.png')
        plt.clf()
...
def training_outputs_epoch(diffusion, x, est, noisy, epoch, row_size, ema, args, save_imgs=False, save_vids=False):
    """
    Saves video & images based on args info
    :param diffusion: diffusion model instance
    :param x: x_0 real data value
    ...
    if save_imgs:
        ...
        plt.savefig(f'./diffusion-training-images/ARGS={args["arg_num"]}/STEP={epoch}_epoch.png')
        plt.clf()
...
def main():
    ...
    sys.argv = ['diffusion_training.py', '28']
    if len(sys.argv[1:]) > 0:
        files = sys.argv[1:]
    else:
...
if __name__ == '__main__':
    ...
    main()
    anomalous_validation_1(shorten=True,DATASET_PATH = './dataset/EdinburghDataset/Anomalous-T1')
    anomalous_metric_calculation(shorten=True,DATASET_PATH = './dataset/EdinburghDataset/Anomalous-T1')
