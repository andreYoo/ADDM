    ...
    def get_patients(options):
        ...
        for n, minc_folder in enumerate(minc_folders):
            ...
            if options.filterProtocol:
                _regex = BRAINWEB.PROTOCOL_MAPPINGS[options.filterProtocol] + ".mnc"
            else:
                _regex = "*.mnc"
            ...
            for f, fname in enumerate(_files):
                ...
                if patient['type'] == 'NORMAL':
                    patient['groundtruth_filename'] = os.path.join(options.dir, options.folderGT, 'normal.mnc')
                elif patient['type'] == 'MILDMS':
                    patient['groundtruth_filename'] = os.path.join(options.dir, options.folderGT, 'mild_lesions.mnc')
                elif patient['type'] == 'MODERATEMS':
                    patient['groundtruth_filename'] = os.path.join(options.dir, options.folderGT, 'moderate_lesions.mnc')
                elif patient['type'] == 'SEVEREMS':
                    patient['groundtruth_filename'] = os.path.join(options.dir, options.folderGT, 'severe_lesions.mnc')
